...
This produces a self-contained `target/jp/out/MacPad++-1.0.0.dmg` with its own bundled Java runtime. Open the DMG and drag **MacPad++.app** into Applications. Launch it once so macOS registers it, and MacPad++ will appear under Finder's right-click **Open With**.
...
Proprietary - all rights reserved. See [LICENSE](LICENSE). The source is published for viewing only and may not be used, copied, modified, or redistributed without written permission. Bundled dependencies keep their own licenses (RSyntaxTextArea BSD-3-Clause, FlatLaf Apache-2.0, Lombok MIT).
